...
const GroupModal = ({ modal, roles, refresh, userInfo, disabled }) => {
  let timeout;
  let currentValue;
  ...
  useEffect(() => {
    ...
    const handleApply = async formData => {
      ...
      else if (roles.includes(ROLE_TYPE.MASTER)) recordAction = 'Create group';
    ...
    const renderTitle = () => {
      ...
      if (modal.modalData) {
        title = `Edit ${title}`;
      } else if (roles.includes(ROLE_TYPE.MASTER)) {
        title = `Create ${title}`;
      } else {
        title = `Apply ${title}`;
      }
