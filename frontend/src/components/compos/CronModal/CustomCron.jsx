// cron轉碼：因有限制可選擇時段,故未用套件
// 限制時段可參考DATEOPTIONS選項
// 此modal可將UI轉Cronjob碼 亦可 將Cronjob碼轉UI
...
const CustomCron = ({
  ...
  const handleGetCron = () => {
    ...
    if ((selectTime !== '') && (selectTime.length === 1)) {
      // 只有選一個時間
      let indexTime = -1;
      if (selectDate === DATEOPTIONS.Day.key) cron = `0 ${selectTime[0].substr(0, 2)} * * *`;// DAY
    else if (selectDate === DATEOPTIONS.Week.key) { // Week
        indexTime = optionDate.indexOf(selectTime[0]);
        cron = `0 12 * * ${indexTime}`;
      } else if (selectDate === DATEOPTIONS.Month.key) { // Month
        indexTime = optionDate.indexOf(selectTime[0]) + 1;
        cron = `0 12 ${indexTime} * *`;
      }
    } else if (selectTime.length > 1) {
      // 選多個時間
      if (selectDate === DATEOPTIONS.Day.key) cron = returnNumForDay(selectDate);
  ...
  useEffect(() => {
    if (getUIValue !== undefined && getUIValue.length !== 0) handleGetUI(getUIValue);
  }, [modalVisible]);
