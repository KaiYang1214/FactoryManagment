...
import SqlEditor from '@/components/SqlEditor/SqlEditorForDiagram';
...
export class OutputSettingNodeWidget extends React.Component {
  ...
  getCurrAllEntity = () => {
    return this.props.engine.model
      .getNodes()
  ...
  generatePort = port => {
    const portCom = (
      <PortWidget engine={this.props.engine} port={port} key={port.getID()}>
        <Style.outputSettingPort />
      </PortWidget>
    );
    const label = <Style.outputsettingPortLabel />;

    return (
      <Style.outputsettingPortLabelContainer>
  ...
  render() {
    return (
      ...
      >
        ...
        >
          <Style.outputSettingTitle>
            <Style.outputSettingTitleName>
          ...
          <Style.outputSettingPortContainer>
